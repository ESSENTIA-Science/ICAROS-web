...
| **횡단 목록에 새 기능을 등록했는지 사람이 기억하게 두지 말 것** | `hasReferences()` 에 `page_panels` 를 빠뜨려 **정리 cron 이 살아 있는 랜딩 사진 4장을 지웠다**(D28). 빠뜨려도 경고가 없고 증상은 몇 주 뒤 "사진이 사라졌다"로만 온다. **스키마가 목록을 검사하게** 만들 것 — `db:verify` 가 `information_schema` 의 실제 FK 와 대조한다. 방향은 **"DB 에 있는데 목록에 없는" 쪽 하나만** |
| **자격증명 불일치는 "어느 쪽이 정답인가"를 먼저 정할 것** | 양쪽 다 바꿀 수 있는 값이라, 확인 없이 한쪽을 건드리면 **불일치가 유지된 채 시간만 간다**. 2026-08-30 공유 RDS 40분 장애에서 ESSENTIA 가 두 번 헛짚었다 — **읽을 수 있는 쪽을 먼저 읽었으면 15분 줄었다.** 우리 IAM 경로도 "토큰이 틀렸나·망이 막혔나·상대가 뭘 바꿨나"가 전부 **타임아웃 하나로** 보인다 |
