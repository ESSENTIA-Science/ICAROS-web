...
## 프레이밍 수정 (2026-08-24) — 카메라 롤 90도

### 문제
독립 검토 실측: 히어로 타깃 박스가 **1360 × 239**(1440×900 기준, 5.7:1)인데 기체는
**0.464 × 1.660 × 0.464** 로 길쭉하다. 세로가 구속조건이 되어 가로를 거의 채우지 못했다.
화면상 약 **200 × 60 px** — 1200px 폭 띠 안의 머리카락 한 올.

### 해결
**모델이 아니라 카메라 up 을 90도 돌렸다** (`config.roll`, `framing.rollUp()`).

모델을 돌리지 않은 이유: `Box3.setFromObject` 는 회전 **전** 박스를 준다.
모델만 돌리면 fit 계산과 실제 렌더가 어긋난다. 기저(right/up)를 돌리면
AABB 8꼭짓점 투영이 그대로 따라오므로 두 곳이 자동으로 일치한다.

### 개선 (해석적 계산, `fitDistance` 실호출)
| 뷰포트 | 박스 | roll=0 | **roll=90** |
|---|---|---|---|
| 1440×900 | 1360×239 | fillX **5.2%** / fillY 82.1% | fillX **36.7%** / fillY 75.1% |
| 1280×800 | 1200×150 | fillX 3.7% / fillY 82.1% | fillX **26.1%** / fillY 75.1% |
| 1920×1080 | 1420×419 | fillX 8.7% / fillY 82.1% | fillX **61.7%** / fillY 75.1% |

가로 채움이 **7배**로 늘고 세로는 82→75% 로 소폭만 줄었다.

### 포스터 재굽기
`config.roll` 을 바꾸면 포스터도 **반드시** 다시 구워야 한다 — 폴백 2단(포스터)에서
1단(3D)으로 승격되는 순간 프레이밍이 이어지지 않으면 그림이 튄다.
```
npx tsx scripts/model/render-poster.ts public/assets/models/icx-2.glb \
  --out public/assets/models/icx-2-poster.png --width 1600 --height 560 --roll 90
```
세로 900×1600 → **가로 1600×560** 으로 바꿨다. 기체가 눕었으니 포스터도 가로여야
`object-fit: contain` 에서 레터박스가 최소가 된다. 38,831 B → **27,742 B**.

`render-poster.ts` 의 `rotateAboutAxis()`(로드리게스)와 `framing.ts` 의 `rollUp()` 은
**같은 식**이다. 한쪽만 고치면 승격에서 튄다.

### 남은 것
- **육안 검증 미완.** 이 세션에 브라우저가 없어 수치로만 확인했다. 아침에 실제로 봐야 한다.
- 1280×800 의 박스가 **1200×150** 으로 특히 납작하다. roll 후에도 fillX 26% 다 —
  히어로 `grid-template-rows` 의 `row-gap`·`padding-block` 을 줄여 박스 높이를 늘리면 더 개선된다.
  다만 그건 조판 판단이라 여기서 하지 않았다.
