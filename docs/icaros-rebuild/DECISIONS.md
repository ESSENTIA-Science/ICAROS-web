...
```
이전:  0.0.0.0/0                    :5432
이후:  AWS us-east-1 EC2 대역 29개   :5432  +  ESSENTIA EC2 보안그룹
```

대역은 추측이 아니라 **RDS 로그에서 관측된 `icaros_app` 클라이언트 IP 124개**를 역산해
그 IP 들이 실제로 속한 CIDR 을 병합한 것이다. 추가 → 제거 순서라 거부 구간은 없었다.

### ⚠️ 잔여 위험 — 이건 완전한 목록이 아니다

**AWS 가 us-east-1 EC2 에 공표한 CIDR 은 294개인데 29개만 들어가 있다.**
보안그룹 인바운드 규칙 상한이 **60개**라서다(294개는 병합해도 233개라 못 들어간다).
29는 타협이 아니라 상한이 강제한 결과다.
...
허용 대역이 여전히 **1,570만 IP** 다. us-east-1 에 EC2 를 띄운 누구든 5432 에 **도달**은 한다.
