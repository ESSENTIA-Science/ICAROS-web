...
### 착수 전 판정 프로브 — **글을 만들지 않고 확인된다**

`GET /api/forum/write-form` 이 `/api/forum/` 아래이고 `requireSignedIn()` 을 타므로,
토큰만 실어 보내면 상태가 갈린다. **첫 시도 전에 이것부터 때린다.**

```
GET https://api.essentia-sci.org/api/forum/write-form
X-Service-Token: <토큰>
```

| 응답 | 의미 |
|---|---|
| **401** | 필터가 안 붙었다 — 환경변수 미설정이거나 토큰 불일치 |
| **200** + `role: "OUTSIDER"` | 필터 붙음 + `svc-icaros` user 행 존재 → **쓰기 가능** |

이게 중요한 이유는 아래 두 실패 모드가 화면에서 구분되지 않기 때문이다.
지금까지 우리가 가진 판정 수단은 "글을 하나 써 본다"뿐이었고, 그건 성공해도 실패해도 뒷정리가 남는다.

### ⚠️ `projects: []` 는 **정상이다** — 이걸 오독하면 다시 보류하게 된다

200 이 떠도 응답의 `projects` 배열은 **비어 있다.** `writeForm()` 이 이렇게 짜여 있다:

```java
List<ProjectOption> taggable = viewer.isMember() ? publicProjectOptions() : List.of();
```

서비스 주체는 `members` 행이 없어 `isMember()` 가 false 라 빈 목록이 온다.
**이건 "프로젝트에 못 쓴다"는 뜻이 아니다.** 작성 폼용 선택지일 뿐이고, 실제 쓰기 권한은
`createPost` → `resolveProjectTag()` 의 서비스 주체 분기에서 **따로** 판정된다.

가장 자연스러운 오독은 "인증은 됐는데 프로젝트 권한이 없다"이고, 그 결말은
"역시 D14 벽이 안 열렸네" 하고 다시 보류하는 것이다. **열려 있는데.**
`projectId` 는 목록에서 고르지 말고 위의 UUID 를 그대로 실어 보낸다.
...
## D26 — 커넥션 포화는 우리가 냈다 (2026-08-27). 로그로 확증됨

> **상태 (2026-08-28): 완화 성공 / 관측 단계.**
> 최대 커넥션 **77 → 36**, FATAL **95 → 0**, 알람 재발화 **0회**.
> `max: 3 → 1` 은 **취소**. PgBouncer 는 **보류**(향후 구조 개선 후보).
> cold fan-out 구조는 **잔존** — 현 수준에서는 추가 완화보다 관측 유지가 맞다.
> 남은 부채 관리는 **`15-infra-debt.md` 가 canonical** 이다.

`essentia_infra` 가 `aws rds download-db-log-file-portion` 으로 PostgreSQL 로그를 꺼냈다.
`log_line_prefix` 가 `%t:%r:%u@%d:[%p]:` 라 **모든 줄에 사용자와 클라이언트 IP 가 찍혀 있었다.**
추측이 끝났다.

### 무엇이 나왔나 — 14시간 FATAL 집계

| 건수 | 주체 | 내용 |
|---|---|---|
| **95** | `icaros_app@essentia` | `remaining connection slots are reserved…` |
| 3 | `[unknown]` | 포트 스캐너 (D27) |
| **1** | `essentia@essentia` | 같은 오류 — **우리가 상대를 밀어냈다** |

`icaros_app` 의 **고유 클라이언트 IP 63개**(54.x·44.x·13.x·100.31.x — Fluid Compute 인스턴스).
시각이 커넥션 스파이크와 정확히 겹친다: 12:27 KST 14건 · 12:30 7건 · **12:32 25건**.

### 🔴 우리가 실제로 낸 피해

```
2026-08-27 03:27:45 UTC : 172.31.35.38 : essentia@essentia :
FATAL: remaining connection slots are reserved for roles with privileges of the "rds_reserved" role
```

`172.31.35.38` 은 **ESSENTIA EC2 의 사설 IP** 다. 12:27:45 KST 에 상대 백엔드가 커넥션을
요청했다가 DB 에 거부당했다. 서비스가 죽지 않은 것은 Hikari 가 상시 10개를 쥐고 있어
기존 커넥션으로 버텼기 때문이고, **그 순간 상대 배포가 돌았으면 앱이 아예 못 떴다**
(상대 배포는 `docker rm -f` → `run` 이라 새 컨테이너가 커넥션 10개를 새로 요청한다).

우리가 6월에 겪은 `53300` 은 우리만의 사고가 아니었다. 같은 일이 상대에게도 간다.

### 원인 — `idleTimeoutMillis` 가 아니다

처음에 감쇠 곡선(68 → 12까지 25분)을 보고 유휴 시간을 의심했는데 **틀렸다.**
진짜 변수는 **동시 인스턴스 수**다. 63개가 살아 있으면 각자 1개씩만 잡아도 이미 63이다.
`idleTimeoutMillis` 는 트래픽이 **멎은 뒤**를 지배하는 값이라 버스트 한복판에서는 아무것도 막지 못한다.

부하가 아니라는 것도 지표가 말한다 — 커넥션이 10 → 68 로 **6.8배** 뛰는 동안
CPU 는 5.4% → 12.2% 였다. `FreeableMemory` 감소분을 추가 커넥션으로 나누면 **커넥션당 ~0.45MB**,
Postgres 백엔드 프로세스의 상주 크기다. **붙고, 거의 아무것도 안 하고, 남아 있었다.**

### 구조가 원인이다

```
서버리스(수평 확장 무제한)  +  직접 Postgres 커넥션(슬롯 고정 ~112)
```

**인스턴스 수가 앱 손에 없으면 인스턴스당 상한으로 총량을 못 묶는다.**
`max: 3` × 63 인스턴스 = 이론상한 **189**, 실사용 가능분 109.

여기에 증폭기가 붙어 있다 — **랜딩 1회 = DB 를 치는 함수 호출 58회.**
사진 한 장이 `_next/image` → `/api/media/<id>` 로 두 겹이고 안쪽이 DB 를 친다.
고유 사진은 5장인데 srcset 변형이 40개다.

### 대응 — 우선순위

| | 무엇 | 효과 | 비고 |
|---|---|---|---|
| 1 | **`/api/media` 가 DB 를 안 치게** | 20회 요청당 DB 트랜잭션 **20 → 3** | ✅ 완료 (`5283919`). 캐시 대상을 이미 `immutable` 인 종류로 한정 — 새 노출 0 |
| 2 | ~~`sizes` 좁히기~~ → **`deviceSizes` 축소** | 변형 **40 → 25** (−37.5%) | ✅ 완료. `sizes="100vw"` 는 정직한 값이라 못 좁힌다 — 대신 변형 개수를 줄였다 |
| 3 | ~~`max: 3 → 1`~~ | 이론상한 189 → 63 | ❌ **하지 않는다.** 1·2 로 충분했다 — 아래 §측정 결과 |
| 4 | PgBouncer | 근본책 | ⏸ **보류.** 당장 도입하지 않고 향후 구조 개선 후보로 남긴다. 재검토 조건은 `15-infra-debt.md` §A5 |

#### 2번 실행 기록 — 화질을 판 것이 아니라 중복을 지웠다

`deviceSizes` 기본값 8개(640·750·828·1080·1200·1920·2048·3840)를 **5개**(640·828·1200·1920·3840)로 줄였다.
...
**상단 3840 은 남겼다** — 1920 화면 DPR 2 가 실제로 그걸 쓰고, 전면 사진에서 그 손실은 보인다.
`imageSizes` 도 8개 → 3개(128·256·384). 쓰는 곳이 `/admin` 썸네일(120·200·320px)뿐이다.

실측(로컬 프로덕션 빌드): 랜딩 1회 `_next/image` 변형 **40 → 25**, 고유 사진은 5장 그대로.
생성 폭도 지정한 5개와 정확히 일치한다.

**`max: 1` 은 공짜가 아니다.** Fluid Compute 는 인스턴스 하나가 동시 요청 여러 개를 처리하는 것이
핵심인데, `max: 1` 이면 그 요청들이 커넥션 하나에 줄 선다. `connectionTimeoutMillis` 가 10초라
부하가 몰리면 **커넥션 문제가 지연 문제로 바뀔 뿐**이다.
1·2번을 먼저 했고 **그것으로 충분했다** — 아래 §측정 결과. 3번은 취소됐다.

트랜잭션 안에서 풀을 다시 잡는 패턴은 **0건**임을 확인했다(전부 `tx` 를 쓴다) — `max` 를 낮춰도
데드락은 나지 않는다.

### 측정 결과 (2026-08-28) — 1·2 로 끝났다. 3번은 하지 않는다

`essentia_infra` 가 1분 해상도로 배포 전후를 대조했다. 표본 1,104개(8/27 14:00 ~ 8/28 08:24).

| | 배포 전 | 배포 후 | |
|---|---|---|---|
| p50 | 10 | **10** | 변화 없음 — 상대 Hikari 상시분 |
| p90 | 13 | 16 | +3 |
| p99 | 28 | 26 | −2 |
| **최대** | **77** | **36** | **−53%** |
| **`icaros_app` FATAL** | **95건** | **0건** | |
| `essentia@essentia` 거부 | 1건 | **0건** | |

**p50 이 10 그대로인 것이 핵심이다.** 그 10은 상대가 상시 쥐는 몫이고, 그 위에 얹히던 우리 몫이
꼬리에서만 잘렸다. **기저는 안 건드리고 꼬리만 깎았다** — 캐시가 정확히 그렇게 동작해야 하는 모양이다.

### "배포 직후 콜드 팬아웃" 가설이 확정됐다

배포 후 30을 넘긴 구간이 **2개뿐이고 둘 다 우리 배포 시각 ±8분** 안이다.
그 뒤 18시간 동안 30 이상이 **0회**. 그리고 그 콜드 팬아웃조차 36으로,
배포 전의 평상시 스파이크(65~77)보다 낮다.

### 3번을 하지 않는 이유 — 우리가 미리 세운 기준 그대로

> 여전히 60을 넘는다 → `max: 1` 진행 / 눈에 띄게 낮아졌다 → 유지하고 관측 계속

**60을 넘긴 적이 없다.** 최대 36이고 경고선까지 24, 위험선까지 49의 여유가 있다.
`max: 1` 은 Fluid Compute 가 인스턴스 하나로 동시 요청을 처리하는 구조라 **커넥션 문제를
지연 문제로 바꾼다.** 그 대가를 치를 이유가 사라졌다.

### ⚠️ 두 변경의 기여도는 가르지 못한다 — 정직하게 적어 둔다

캐시와 변형 축소를 **같이 배포했다.** RDS 지표만으로는 어느 쪽이 얼마인지 안 갈린다.
산수로는 캐시가 지배적일 것 같지만(트랜잭션 −85% vs 변형 −37.5%) **추정이고 검증되지 않았다.**

갈라야 할 필요가 생기면 — 예를 들어 화질 때문에 `deviceSizes` 를 다시 늘리고 싶을 때 —
**한 번에 하나씩 되돌리면서 재는 수밖에 없다.** 지금은 갈릴 필요가 없다. 둘 다 그 자체로 옳은 변경이다.

### 알람

ESSENTIA 가 경고(`DatabaseConnections >= 60`, 5분×2)·위험(`>= 85`, 5분×1) 알람을 붙였고
**메일 도달까지 확인**했다. 13:04 스파이크에서 실제로 울렸다.
우리 주소를 그 SNS 에 넣는 건 상대 사용자 결정 대기다.

### 방법론 — 지표의 창 크기

같은 데이터가 창 크기만 바꿔서 **28 → 77** 이 됐다. `--period 86400` 은 5분 스파이크를 평균으로
눌러 버린다. **양쪽이 각각 한 번씩 이 함정을 밟았다.** 개인 실수가 아니라 도구 기본값이 만드는
함정이므로, 커넥션·메모리처럼 순간값이 중요한 지표는 **1분 해상도로 볼 것.**

---

## 🔴 D27 — RDS 5432 인바운드가 좁혀졌다 (2026-08-27). 완전한 목록이 아니다

포트 스캐너 탐지가 계기다. RDS 로그에 남은 것:

```
66.240.223.214   3건    unsupported frontend protocol 0.0 / 255.255
18.218.118.203   3건    no PostgreSQL user name specified in startup packet
```

프로토콜 버전 `0.0`·`255.255` 를 던져 보는 것은 핑거프린팅이다. 아직 인증 시도는 없다
(사용자명조차 보내지 않는다). **D20 에서 "받아들인 잔여 위험"으로 적어 둔 것이 현실화되는 초기 단계다.**

### 무엇이 바뀌었나

```
이전:    0.0.0.0/0                   :5432
1차:     us-east-1 EC2 대역 29개      (관측 IP 124개에서 역산)
최종:    us-east-1 EC2 대역 59개      :5432  +  ESSENTIA EC2 보안그룹
```

### 🔴 관측 기반 목록의 수명은 **몇 분**이었다

1차(29개)는 그날 관측된 `icaros_app` IP 124개를 역산한 목록이었다. 변경 **직후** 로그에 이게 남았다:

```
04:40:10 UTC : 35.153.176.161 : icaros_app@essentia :
LOG: could not receive data from client: Connection timed out
```

`35.153.176.161` 은 **29개 대역 밖**이다. 우리 인스턴스인데 허용목록에 없었다.
"시간이 지나면 표류한다"고 적어 둔 것이 **몇 분**이었다는 뜻이다.

**교훈: Fluid Compute 의 IP 는 관측으로 고정할 수 없다.** 어제 쓴 대역이 오늘의 대역이 아니다.

그래서 규칙 60개를 꽉 채웠다 (1개는 ESSENTIA EC2 SG 참조라 CIDR 은 59개):

| | CIDR | 허용 IP | us-east-1 EC2 커버리지 |
|---|---|---|---|
| 1차 | 29 | 15,679,488 | 72.9% |
| **최종** | **59** | **21,053,440** | **97.8%** |

슈퍼넷으로 뭉개면 규칙 수를 더 줄일 수 있었지만 허용 IP 가 1억 5,900만으로 뛰고
그중 86.5% 가 AWS 도 아닌 공간이라 버렸다. **초과허용 0** — 실제 AWS 대역만 큰 블록부터 59개.

무중단 교체 순서: 제거 대상을 덮는 상위 대역을 **먼저** 넣고(60 도달), 구 3개를 빼고(57),
잔여 3개를 넣었다(60). 2단계에서 빠진 대역이 1단계 상위 블록에 이미 포함돼 공백이 없다.

### ⚠️ 잔여 위험 — 여전히 완전한 목록이 아니다

**공표된 CIDR 294개 중 59개만 들어간다.** 보안그룹 인바운드 규칙 상한이 **60개**라서다.
59는 타협이 아니라 상한이 강제한 결과이고, **이제 규칙이 꽉 찼다** — 더 넣으려면
다른 걸 빼거나 쿼터 상향을 신청해야 한다.

미포함분은 CIDR 174개 / **465,312 IP (2.2%)**, 대부분 /17·/18 자투리다.

> **Fluid Compute 가 목록에 없는 us-east-1 CIDR 로 확장하면 그 인스턴스만 조용히
> 커넥션 타임아웃이 난다.** 전체 장애가 아니라 **일부 인스턴스만** 실패하는 형태다.

**간헐적 DB 커넥션 타임아웃이 보이면 코드를 의심하기 전에 `essentia_infra` 에 먼저 물을 것.**
증상(타임아웃)과 원인(네트워크 허용목록)이 어긋난다 — D25 의 `configured()` 건과 같은 종류다.
차단된 IP 는 네트워크 단에서 끊기므로 **RDS 로그에 아예 남지 않는다.** 진단은 Vercel 로그의
실패 인스턴스 IP 를 상대에게 넘기는 경로로만 된다.

### 이걸 "해결됐다"로 적지 않는다

허용 대역이 여전히 **2,105만 IP** 다. us-east-1 에 EC2 를 띄운 누구든 5432 에 **도달**은 한다.
인증(IAM 토큰·마스터 비밀번호)이 여전히 유일한 방어선이고, **D17 ⑤ "로그인 시도 제한이 없다"는
그대로 유효하다.**

정확한 표현은 이것이다 — **공격 표면이 인터넷 전체에서 us-east-1 EC2 로 좁혀졌다.** 사라진 것이 아니다.
