...
| | 무엇 | 효과 | 비고 |
|---|---|---|---|
| 1 | **`/api/media` 가 DB 를 안 치게** | 58회 중 대부분이 커넥션을 안 만든다 | `landing` 미디어는 immutable. **호출 수를 줄이는 것보다 이게 먼저다** |
...
| 4 | PgBouncer | 근본책 | 구성요소가 는다. RDS Proxy 는 이 규모에 월 $87 수준이라 비용이 안 맞는다 |
...
부하가 몰리면 **커넥션 문제가 지연 문제로 바뀔 뿐**이다. 1·2번을 먼저 한다.
